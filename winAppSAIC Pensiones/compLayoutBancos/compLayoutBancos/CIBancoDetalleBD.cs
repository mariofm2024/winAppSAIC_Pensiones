...
{
    public static class CIBancoDetalleBD
    {
        ...
        /// <returns>Objeto Lista CIBanco</returns>
        public static List<CIBanco> ObtenerInformacionBancariaEmpleado(DataTable objTable)
        {
            List<CIBanco> objInfoBancariaEmpleadoList = new List<CIBanco>();
            CIBanco objInfoBancariaEmpleado = null;
            ImporteDetalle = 0;     //inicializar propiedad
            try
            {
                foreach (DataRow row in objTable.Rows)
                {
                    objInfoBancariaEmpleado = new CIBanco();
                    objInfoBancariaEmpleado.TipoPago = row[0].ToString();
                    objInfoBancariaEmpleado.Importe = (decimal)row[1];
                    objInfoBancariaEmpleado.NoCuentaBanco = row[2].ToString();
                    objInfoBancariaEmpleado.Beneficiario = row[3].ToString();
                    objInfoBancariaEmpleado.RFC = row[4].ToString();
                    objInfoBancariaEmpleado.DescPago = row[5].ToString();
