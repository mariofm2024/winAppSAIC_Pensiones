...
namespace compLayoutBancos
{
    public static class BanorteDetalleBD
    {
        ...
        public static List<Banorte2016> ObtenerInformacionBancariaEmpleado2016(DataTable objTable, string strFechaAplicacion)
        {
            List<Banorte2016> list = new List<Banorte2016>();
            Banorte2016 item = null;
            try
            {
                foreach (DataRow row in objTable.Rows)
                {
                    item = new Banorte2016
                    {
                        Operacion = row[0].ToString(),
                        ClaveID = row[1].ToString(),
                        CuentaOrigen = row[2].ToString(),
                        NoCuentaBanco = row[3].ToString(),
                        Importe = (decimal)row[4],
                        Referencia = row[5].ToString(),
                        Descripcion = row[6].ToString(),
                        RFCOrdenante = row[7].ToString(),
                        IVA = row[8].ToString(),
                        FechaAplicacion = strFechaAplicacion,
                        InstruccionPago = row[10].ToString(),
                        ClaveTipoCambio = row[11].ToString(),
                        IdLoteCargaNomina = Convert.ToInt64(row[12])
                    };
                    list.Add(item);
                }
            }
